	...
	function dispatch(){
		if (array_key_exists("$this->ms",$this->msmoduler)){
			if ($this->adgang->sjekkAdgang($this->ms) > 0) {
                ...
                // Sjekker om 'stille' variabel er satt i modulvisning, og viser ingen output dersom denne er satt, viser feilmelding dersom den ikke er satt.
			    if (array_search('stille', $this->vars) === false) throw new AdgangsException('Dispatcher: Innlogget bruker har ikke adgang til å vise modulen "' . $this->ms . '".');
			}
		} else {
